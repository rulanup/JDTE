...
| FTB Ultimine | `2101.1.13` | 可选扳手批量旋转和搬运 |
...
抢夺和锋利是独立的专用升级物品，不属于 `UpgradeType` 通用枚举。生物粉碎机在专用槽中分别允许最多 6 张；战利品制造机使用 `LootFabricatorUpgradeItemStackHandler`，在 8 个通用升级槽之外提供最多 3 张抢夺升级的限制逻辑。
...
- 新增高级物品拾取器：复用 JDT 模型和界面，提供 8 个标准升级槽，按区块索引在掉落物加入世界前事件驱动收集；支持部分插入后的剩余物正常生成，且不产生物品流动粒子。玩家破坏容器时，单槽达到可配置阈值（默认 10M）会通过公开能力在生成掉落实体前直传；触发槽位无法完整转移时取消破坏。朝向目标提供 AE2 `ME_STORAGE` 时会使用长整型数量直接写入，同时兼容 ExtendedAE 扩展接口和超大接口。
- 高级物品拾取器只允许范围和过滤升级，服务端槽位校验与 GUI 兼容提示共用同一规则。
- 蚀空合金扳手加入 `c:tools/wrench` 和 `c:wrenches` 通用标签，由 AE2 等兼容模组使用原生扳手钩子完成旋转和快捷拆除。
- 蚀空合金扳手恢复标准手持模型结构，同时继续使用 JDTE 专用贴图。
- 新增蚀空合金扳手双角点范围框选：复用 JDT 范围预览、实时显示尺寸、支持将同一选择写入多台机器、服务端校验半径/偏移上限，并在创造模式阻止扳手破坏方块。
- 新增升级卡蹲下右键直接插入及 FTB Ultimine 批量插入，覆盖普通升级、生物粉碎机专用升级和战利品制造机抢夺升级。
- 新增 Jade 已安装升级显示，按物品聚合普通和专用升级数量。
- FTB Ultimine 依赖更新为 CurseMaven 文件 `8231400`，修复新版 API 兼容，并新增蚀空合金扳手合成配方。
