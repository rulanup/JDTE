...
Factory Packer v2 records use block-state NBT, mod-aware block entity move data, recursively saved root entities, scheduled block/fluid ticks, and source transform metadata. The v1 decoder remains supported without coordinate remapping. The `data/jdte/tags/block/factory_packer_blacklist.json` tag rejects unsafe blocks and can be extended by datapacks. Rejections are collected within the existing bounded capture pass and report up to 16 localized names, registry IDs, and source or transformed destination positions; players with command level 2 can click coordinates to teleport.
...
- Factory Packer permission probes now run after entity detachment, preventing simulated break events from destructively removing Logistics Networks nodes before capture.
- Factory Packer base X/Y/Z radius now defaults to 10, running source topology receives up to three bounded recaptures, and fully selected Mekanism fission reactors are safely stopped before authoritative capture while partial selections are rejected with their full bounds.
...
Factory Packer's temporary detailed server diagnostics are gated by the JVM property `-Djdte.factoryPackerDebug=true`. They are disabled by default and log phase transitions, source mismatches, recapture package IDs, and operation results with the `[FactoryPacker/Debug]` prefix.
