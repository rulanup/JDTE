...
public class ExtendedGelGeneratorContainer extends GelGeneratorContainer {
    ...
    public ExtendedGelGeneratorContainer(int windowId, Inventory playerInventory, BlockPos blockPos) {
        super(JDTEMenus.EXTENDED_GEL_GENERATOR.get(), windowId, playerInventory, blockPos);
        addPlayerSlots(player.getInventory());
    }

    @Override
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(player.level(), pos), player, JDTEBlocks.EXTENDED_GEL_GENERATOR.get());
    }
}
