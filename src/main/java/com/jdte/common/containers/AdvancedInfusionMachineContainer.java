...
public class AdvancedInfusionMachineContainer extends InfusionMachineContainer {
    ...
    public AdvancedInfusionMachineContainer(int windowId, Inventory playerInventory, BlockPos blockPos) {
        super(JDTEMenus.ADVANCED_INFUSION_MACHINE.get(), windowId, playerInventory, blockPos);
        addPlayerSlots(player.getInventory());
    }

    @Override
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(player.level(), pos), player, JDTEBlocks.ADVANCED_INFUSION_MACHINE.get());
    }
}
