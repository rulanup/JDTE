...
public final class MachineScreenJeiGuiHandler implements IGuiContainerHandler<BaseMachineScreen<?>> {
    ...
    public List<Rect2i> getGuiExtraAreas(BaseMachineScreen<?> screen) {
        if (screen instanceof MachineScreenAreaProvider provider) {
            return provider.jdte$getMachineScreenAreas();
        }
        return Collections.emptyList();
    }

    @Override
    public Collection<IGuiClickableArea> getGuiClickableAreas(BaseMachineScreen<?> screen, double guiMouseX, double guiMouseY) {
        if (screen instanceof InfusionMachineScreen<?>) {
            return List.of(IGuiClickableArea.createBasic(
                    INFUSION_PROGRESS_X, INFUSION_PROGRESS_Y,
                    PROGRESS_WIDTH, PROGRESS_HEIGHT,
                    InfusionRecipeCategory.RECIPE_TYPE));
        }
        if (screen instanceof GelGeneratorScreen<?>) {
            GuiUpgradeLayoutConfig config = GuiUpgradeLayoutConfig.getInstance();
            return List.of(IGuiClickableArea.createBasic(
                    config.getGelGenProgressArrowX(), config.getGelGenProgressArrowY(),
                    PROGRESS_WIDTH, PROGRESS_HEIGHT,
                    GelGeneratorRecipeCategory.RECIPE_TYPE));
        }
        if (screen instanceof AdvancedPotionBrewerScreen) {
            GuiUpgradeLayoutConfig config = GuiUpgradeLayoutConfig.getInstance();
            return List.of(IGuiClickableArea.createBasic(
                    config.getPotionBrewerArrowX(), config.getPotionBrewerArrowBottomY() - 28,
                    9, 28,
