...
The Large Greenhouse is a single-placement **3×3×2** machine. It provides nine independent production lanes, reuses every plant template, and consumes each recipe's required fluid (Time Fluid by default) plus FE.
...
3. Supply FE and the recipe's required fluid (Time Fluid by default), then select a speed from 1-32x.
...
| Base cost | 10 FE per harvest; fluid depends on the plant recipe |
| Large-structure bonus | Each lane runs at 9x production speed; batched recipe-fluid cost is divided by nine |
...
JEI shows the template, base FE, required recipe fluid, and preview outputs. Clicking the growing-bed progress area also opens the category.
...
The controller occupies the front-center block of the bottom layer; the other 17 blocks are structure parts. Every base-layer face accepts templates, recipe-required fluid (Time Fluid by default), and FE and exposes product extraction. Using any part opens the controller. Breaking any part removes the whole structure and returns exactly one Large Greenhouse item.
