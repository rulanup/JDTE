...
- **Fixed — Draconic Stabilized Spawner**: The `StabilizedSpawnerLogic` accessor now declares the exact `TileStabilizedSpawner` field type so the mixin applies; the previous `Object` descriptor matched no candidate, failed mixin application, and crashed on spawner placement. `SpawnerMixin$DraconicSpawnerMixin` is registered in `mixins.jdte.json`, reflection failures fall back to the default spawn count, and a contract test covers the metadata.
- **Easter egg artwork**: The hidden `2i` slot texture swap now ships the full 512x512 artwork rendered through `EasterEggMipmapTexture` (CPU mip chain 512→16 with trilinear filtering), replacing the 0.6.0 placeholder texture.
...
- **Fixed — AE acceleration and true multipliers**: Molecular Assemblers without a vanilla block-entity ticker are now recognized. Nominal `X` includes the native tick, so continuous accelerators request exactly `X - 1` additional cycles instead of multiplying the request by the configured duration. Full Grid work no longer becomes a fake 1024x behind the ordinary 4096-execution budget, and insufficient FE or Time Fluid rejects the whole batch without execution or charge instead of partially reducing it.
- **New — JDTE-AE full-grid addon**: The independent `jdte-ae` Jar repeats complete AE2 Server Start, Level Start, Level End, and Server End lifecycles, allowing Molecular Assemblers, crafting CPUs, Pattern Providers, and lifecycle-compatible addon devices to advance together. Any covered online, fully booted node selects its entire Grid; multiple covered nodes on that Grid do not duplicate one accelerator's contribution. Nominal 1024x executes exactly 1023 additional Grid lifecycles per real tick and may substantially increase MSPT. Matching JDTE-AE and JDTE versions plus AE2 19.2.17+ are required on both client and server. ExtendedAE is included only on the development verification runtime and is not a published hard dependency.
- **Fixed — AE virtual tick identity**: JDTE-AE now assigns thread-local monotonic logical tick stamps across native and virtual lifecycle observations. Adjacent real frames can no longer reuse the same AE crafting-change stamp, while AE2's global tick counter remains untouched and the logical clock is cleared when the server stops.
...
- **Fixed**: Client-only loot and spawn-egg sync handlers no longer risk loading client classes on dedicated servers.
...
0.6.0 系列热修复版本，适用于 Minecraft 1.21.1、NeoForge 21.1.216+、JDT 1.5.7+ 和 Java 21。完整说明：[0.6.0-fix1 发布说明](docs/releases/0.6.0-fix1.md)。
...
- **修复 — Draconic 稳定刷怪笼**：`StabilizedSpawnerLogic` 访问器改为声明确切的 `TileStabilizedSpawner` 字段类型以使 mixin 正常应用；此前 `Object` 描述符匹配不到候选成员，导致 mixin 应用失败、放置刷怪笼即崩溃。`SpawnerMixin$DraconicSpawnerMixin` 已注册进 `mixins.jdte.json`，反射失败时回退默认生成数量，并由契约测试覆盖元数据。
- **彩蛋材质**：隐藏的 `2i` 槽位贴图切换升级为正式 512x512 材质，经 `EasterEggMipmapTexture`（CPU mipmap 链 512→16 + 三线性过滤）渲染，替换 0.6.0 的占位贴图。
...
- **修复 — AE 加速与真实倍率**：现在可以识别没有原版方块实体 ticker 的分子装配室。标称 `X` 倍包含原生 tick，持续加速器只请求 `X - 1` 个额外周期，不再错误乘以配置持续秒数；完整 Grid 工作也不再受普通 4096 次执行预算限制而形成“假 1024x”。FE 或时间流体不足时整批不执行且不扣费，不再部分缩小倍率。
- **新增 — JDTE-AE 完整 Grid 附属**：独立 `jdte-ae` Jar 重复执行完整的 AE2 Server Start、Level Start、Level End 和 Server End 生命周期，使分子装配室、合成 CPU、样板供应器及遵循该生命周期的附属设备共同推进。范围命中任一在线且已启动的节点即可选择整个 Grid，同 Grid 多节点不会重复计算同一台加速器；标称 1024x 每真实 tick 恰好执行 1023 个额外 Grid 生命周期，并可能显著提高 MSPT。客户端和服务端都必须安装相互匹配的 JDTE-AE 与 JDTE，以及 AE2 19.2.17+。ExtendedAE 只用于开发验证运行时，不是发布硬依赖。
...
- **新增**：生物粉碎机现已兼容龙之研究的稳定刷怪笼。粉碎机放在稳定刷怪笼正上方时，会消费其原生生成周期（尊重档位的玩家需求与延迟时序），按档位生成数量换算掉落与经验流体，并重置刷怪笼计时而不生成实体。与现有的混沌守护者支持一样，依赖可选的龙之研究模组。
...
- **修复**：战利品与刷怪蛋客户端同步处理不再导致专用服务器加载客户端类。
